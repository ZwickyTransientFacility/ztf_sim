...
def calc_stats(sim_name):
    ...
    stats['Mean Time Between Exposures (s)'] = df.slewTime.mean()
    stats['Mean Slew Distance (deg)'] = np.degrees(df.slewDist.mean())

    stats['90% Time Between Exposures (s)'] = np.percentile(df.slewTime, 90)
    stats['90% Slew Distance (deg)'] = np.percentile(
        np.degrees(df.slewDist), 90)
    ...
    # TODO: make series of diagnostic plots and put them in a run_name
    # directory
